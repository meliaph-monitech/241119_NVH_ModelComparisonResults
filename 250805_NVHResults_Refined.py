...
def load_and_plot_csv_with_highlights(file, summary_df, selected_label_column, hide_ok_segments=False):
    ...
    # Plot bead highlights with train/test differentiation
    for pred in prediction_order:
        if hide_ok_segments and pred in ["OK", "OK-like"]:
            continue  # Skip OK and OK-like if toggled off
        ...
        for _, row in class_rows.iterrows():
            ...
            is_test = row.get("is_test", False)  # Train/Test info
            ...
            hover_template = (
                f"Bead Number: {bead_number}<br>Class: {prediction}"
                f"<br>Set: {'Test' if is_test else 'Train'}"
            )

            line_style = "dash" if is_test else "solid"
            opacity = 1.0 if is_test else 0.4
    ...
    fig.update_layout(
        title="Bead-Level NVH Data Visualization (Train vs Test)",
        xaxis_title="Index",
...
# --- Streamlit UI ---
st.title("Bead-Level NVH Data Classification Viewer (Train/Test Visualized)")
...
if uploaded_zip:
    ...
    if csv_files:
        ...
        # Option to hide OK/OK-like segments
        hide_ok_segments = st.checkbox("Hide OK and OK-like segments", value=False)

        if st.button("Plot Data"):
            with zipfile.ZipFile(uploaded_zip, 'r') as zip_ref:
                with zip_ref.open(selected_file) as file:
                    load_and_plot_csv_with_highlights(file, summary_df, selected_label_column, hide_ok_segments)
    else:
